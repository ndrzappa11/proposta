...
public class API_4 {
	...
	// api di stampaggio della polizza inserita tramite la api3
	...
	public void genera(API_3 oggetto, int k) throws Exception {
		Connection conn =null;
		conn = SaveMySQL.getDBConnection();
		int idpolizza = getId_Polizza(k, oggetto.getId_macchinario(), conn);
		setData_inizio();
		...
		this.massimale = getMassimale(idpolizza, conn);
		this.premio = getPremio(idpolizza, conn);
	}
	...
	public double getPremio(int idpol, Connection conn) throws SQLException {
	...
	public double getMassimale(int idpol, Connection conn) throws SQLException {
		String sql = "SELECT * FROM polizza WHERE idpolizza = '"+idpol+"';";
	...
	public int getId_Polizza(int user, int macchinario, Connection conn) throws SQLException {
		...
		int var = -2;
		try {
			 var = SaveMySQL.getInt(sql, varRicercata, conn);	//metodo all'interno della classe SaveMySQL per non dover riscrivere sempre il codice
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	////////////////////////////////
	// metodi senza la connessione come dato in input
	...
	// genera/setta un oggetto polizza
	...
	public int numPolizze(int id_user, Connection conn) throws SQLException {
		Statement st;
		ResultSet rs;
		Statement stmt = null;
		int i =0;
